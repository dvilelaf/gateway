...
Ethereum/EVM and Solana use the configured `nodeURL`, with standard confirmation and generic rate-limit handling.
...
```bash
# Ensure RPC endpoints are configured in the chain network templates
# Raydium/Meteora/PancakeSwap-Sol require a configured Solana RPC
# Uniswap/PancakeSwap require a configured Ethereum RPC
...
│       └── ...                 # Solana uses the configured nodeURL
...
- **RPC access**: Ethereum/EVM and Solana use their configured `nodeURL`
