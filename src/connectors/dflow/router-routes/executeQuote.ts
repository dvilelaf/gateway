...
export async function executeQuote(
  ...
  // wallet-type-aware chokepoint (local keypair / Ledger / Swig delegate) — no per-wallet-type
  // branching here. NOTE: DFlow routes through many programs, so a Swig role that restricts
  // programs will block it; a Swig wallet used with DFlow needs a token-cap-only role
  // (per-mint caps still bound the blast radius). See src/wallet/swig/README.md.
  logger.info(
