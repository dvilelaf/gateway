...
export async function executeQuote(
  ...
  // Ledger / Swig delegate) — no per-wallet-type branching here. NOTE: aggregator routes
  // traverse many programs, so a Swig role that restricts programs will block them; a Swig
  // wallet used with OKX needs a token-cap-only role (per-mint caps still bound the blast
  // radius). See src/wallet/swig/README.md.
  logger.info(
