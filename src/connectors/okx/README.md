...
- **Swig wallets:** aggregator routes traverse many programs, so a Swig role that restricts
  programs will block them; use a token-cap-only role (see `src/wallet/swig/README.md`).
